...
def find_perpendicular_points2(y_hat, X_new,x0,y0):
    perpendicular_points = []
    p0 = (x0, y0)
    for i in range(0, len(X_new)):
        y = 2*y_hat[i] - y0
        x = (y-y_hat[i])*(x0-X_new[i])/(y0-y_hat[i]) + X_new[i]
        perpendicular_points.append([p0, (round(x), round(y))])
    return perpendicular_points



def uncurve_text_tight(image, X_new, y_hat, x0, y0, d):
    ...
    #X_new = X_new[::-1]
    #y_hat = y_hat[::-1]
    X_new = [i + image.shape[1] / 2 for i in X_new]
    y_hat = [i + image.shape[0] / 2 for i in y_hat]
   ...
   # print("DDDD: ", d)
    ...
    dewarp_image = np.zeros(((2*w+1), len(X_new)), dtype=np.uint8) + 255
    #print("Shirina: ", len(X_new))
    # Calculate perpendicular points

    perpendicular_points = find_perpendicular_points2(y_hat, X_new, int(x0 + image.shape[1]/2), int(y0 + image.shape[0]/2))
    my_iter = 0
    ...
    for points in perpendicular_points:
        ...
        dewarp_image[:, my_iter] = reshape_array_with_interpolation(np.array(pixel_values), (2*w+1), kind='linear')
        my_iter += 1
    ...
    #dewarp_image = cv2.morphologyEx(dewarp_image, cv2.MORPH_CLOSE, kernel)
    #dewarp_image = cv2.threshold(dewarp_image, 230, 255, cv2.THRESH_BINARY)[1]
    dewarp_image = cv2.flip(dewarp_image, 0)

    # print("dewarp")
    # print(dewarp_image)
    crop_image = dewarp_image[int(w - 2*d -5):int(w + 2*d +5), 0:len(X_new)-1]
    ...
    return crop_image
    #cv2.imwrite(f"dataset/output_image/{len(X_new)}.jpg", dewarp_image)
