...
def generate_text_with_even_spaces(total_length, min_letters, max_letters, min_word_length=6, max_word_length=10):
    ...
    while current_letters < num_letters:
        ...
        word = ''.join(random.choices([chr(code) for code in range(ord('А'), ord('Я') + 1) if chr(code) != 'Й'], k=word_length))
        words.append(word)
        current_letters += word_length

    #num_spaces = total_length - num_letters
    num_spaces = random.randint(12, 17) #######random.randint(8, 15)
...
def generate_seal(image_size=(256, 256)):
    ...
    draw_text_on_circle(image, center, outer_radius - 10, second_text, font, text_color='blue', start_angle_deg=270, clockwise=True, flip=True)
